...
- Keep a floating cart summary above the persistent four-item navigation. The navigation labels remain `首页 / 选茶 / 茶会 / 我的`.
- Tapping the populated floating cart summary opens a bottom sheet containing the selected products, quantity controls, removal, total, and checkout action; do not use the generic demo-message sheet for this cart interaction.
- In the cart sheet, keep product rows inset from both rounded screen edges, keep quantity controls fully visible inside the right edge, use a comfortably visible remove icon, and keep the total/checkout footer above the iOS home-indicator safe area.
...
- Cart state persists across tab changes and browser reloads until checkout succeeds. `去结算` opens a full confirmation flow with delivery choice, product review, member discounts, payment summary, and an order-success state; only the successful-payment action clears the cart.
